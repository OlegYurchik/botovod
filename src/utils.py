...
def convert_to_text(func):
    def wrapper(agent, message):
        if message.text is None:
            raise NotPassed
        return func(agent, message.chat.id, message.text)
    return wrapper
...
def convert_to_attachments(func):
    def wrapper(agent, message):
        ...
        if not attachments:
            raise NotPassed
        return func(agent, message.chat, attachments)
    return wrapper
...
def convert_to_images(func):
    def wrapper(agent, message):
        if not event.images:
            raise NotPassed
        return func(agent, message.caht, message.images)
    return wrapper
...
def convert_to_audios(func):
    def wrapper(agent, message):
        if not event.audios:
            raise NotPassed
        return func(agent, message.chat, message.audios)
    return wrapper
...
def convert_to_videos(func):
    def wrapper(agent, message):
        if not message.videos:
            raise NotPassed
        return func(agent, message.chat, message.videos)
    return wrapper
...
def convert_to_documents(func):
    def wrapper(agent, message):
        if not message.documents:
            raise NotPassed
        return func(agent, message.chat, message.documents)
    return wrapper
...
def convert_to_locations(func):
    def wrapper(agent, message):
        if not message.locations:
            raise NotPassed
        return func(agent, message.chat, message.locations)
    return wrapper
...
def only_regexp(expression):
    def decorator(func):
        def wrapper(agent, message):
            ...
            if not match:
                raise NotPassed
            return func(agent, message.chat, *match.groups())
        return wrapper
    return decorator
