...
def only_chat(chat: Chat, cls: type = Agent, is_dialog: bool=False):
    def decorator(func: Callable):
        @wraps(func)
        def func_wrapper(agent: Agent, chat: Chat, message: Message,
                         follower: Optional[Follower]=None, *args, **kwargs):
...
def start_dialog(dialog_cls: type, agent: Agent, chat: Chat, message: Message,
                 follower: Follower):
    follower.set_dialog(dialog_cls.__name__)
    return dialog_cls(agent, chat, message, follower)


async def start_async_dialog(dialog_cls: type, agent: Agent, chat: Chat, message: Message,
                             follower: Follower):
